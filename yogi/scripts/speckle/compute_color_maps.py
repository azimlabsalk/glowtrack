...
def compute_and_save_color_map(args):
    ...
    # render color map 
    color_map = compute_color_map(img.shape[0:2], image_features, template_features, image_matched_idxs, template_matched_idxs, rot)
    print(f'saving output_arr: {output_arr}')
    np.save(output_arr, color_map)
    # save image
    color_map_image = compute_color_map_image(image, color_map)
    plt.figure(figsize=(10, 10))
    plt.imshow(color_map_image)
    kp, des = image_features
    points = np.array([kp[idx].pt for idx in image_matched_idxs])
    if len(points.shape) == 2 and points.shape[0] > 0:
        plt.scatter(points[:, 0], points[:, 1])
    print(f'saving output_image: {output_image}')
#     plt.imsave(output_image, color_map_image)
    plt.axis('off')
...
if __name__ == '__main__':
    ...
    with Pool(n_cpus) as p:
        p.map(compute_and_save_color_map, args)
