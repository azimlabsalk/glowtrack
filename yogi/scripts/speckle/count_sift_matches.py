...
def count_matches(args):
    (template_uv_image, image) = args
    # load image and sift features
    print('load image sift features')
    image_features = sift_list_from_file(image.replace('png', 'key'))
    # load template sift features
    ...
    try:
        (image_idxs, template_idxs) = get_covered_idxs(image_features, template_features)
...
def main(image_dir, template_uv_image, counts_file):
    """compute_labels
    
    \b
    IMAGE_DIR - contains uv speckle images in PNG format
    \b
    TEMPLATE_UV_IMAGE - a uv speckle image path to be used as template
    \b
    COUNTS_FILE - output txt file 
    """
    ...
    for image in image_list:
        args.append((template_uv_image, image))

    n_cpus = multiprocessing.cpu_count()    

    with get_context("spawn").Pool(n_cpus) as p:
        idx_lists = p.map(count_matches, args)
