...
def get_covered_idxs(image_features, covering_features, template2image=True):
    if template2image:
        ...
        matches = compute_matches(image_features,
                                      ...
                                      epsilon=50,
                                      delta=50)
        idxs = [match.queryIdx for match in matches]
    return idxs 


def render_keypoints(kps, label=None, zorder=1.0):
    ...
    plt.scatter(x_lst, y_lst, label=label, zorder=zorder)
...
def compute_and_save_covering(args):
    ...
    feature_idxs = [get_covered_idxs(image_features, features) for features in covering_features]
    # render covering matches on image
    plt.figure(figsize=(10, 10))
    plt.imshow(img, cmap='gray')
    (kp, _) = image_features
    for (rank, idxs) in enumerate(feature_idxs):
        label = f'image {rank}'
        zorder = 1 / (1 + rank)
        render_keypoints([kp[idx] for idx in idxs], label=label, zorder=zorder)
    plt.axis('off')
