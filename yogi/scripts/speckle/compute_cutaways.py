...
def compute_and_save_cutaway(img_in, cut_fraction, img_out):
    img = load_image(img_in)
    img = img[0:1000, :]

    uv = load_image(img_in.replace('red', 'uv'))
    uv = uv[0:1000, :]
    
    h = img.shape[0]
    cut_row = np.clip(h * cut_fraction)

    img[0:cut_row, :] = uv[0:cut_row, :]
